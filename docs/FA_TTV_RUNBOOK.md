...
> ⚠️ **توجه مهم (به‌روزرسانی):** این سند بر اساس شاخهٔ `codex` این مخزن نوشته شده که توکنایزر **espeak/۱۷۸** دارد.
> پروژهٔ واقعی شما `perfect_project` است که توکنایزر **کاراکتری ۲۴۶ نمادی** دارد و با این رویکرد فرق می‌کند.
> برای اجرای واقعی، از **`docs/RAHNAMA_NAHAYI_FA.md`** و نوت‌بوک **`notebooks/FA_TTV_Colab_perfect_project.ipynb`** استفاده کنید. این سند فقط برای رویکرد espeak/۱۷۸ معتبر است.
